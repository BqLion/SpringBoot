...
public class IndexController {
    ...
    public String index(HttpServletRequest request,
       ...
       if(cookies != null && cookies.length != 0)                                             //加一句判断。昨天正常今天就不能访问主页了，抛nullpointer exception。
        ...
        List<QuestionDTO> questionList = questionService.list();        //这里希望返回一个QuestionDTO,一般DTO用mapper返回，
        if(questionList!=null) {
            model.addAttribute("questions", questionList);                    //QuestionMapper只能返回Question model.
        }
